...
if (.Platform$OS.type=="windows") {
  setwd("C:/Users/slubinga/Dropbox/Dissertation_A/MMC-Dissertation/Analyses/DCE Designs/mainsurvey/")
} else {
  setwd("/Users/slubinga/Dropbox/Dissertation_A/MMC-Dissertation/Analyses/DCE Designs/mainsurvey/")
}

# get functions we need
# source("base_mnl_functions.R")
source("clean_data_functions.R")

# Get the data
d <- read.csv("maindata_full3.csv", header=T)    # use for csv files
...
## children
d$nochild=d$children_fathered==0
...
d$occup_other=as.numeric(d$occupation_n %in% c(5,6))
...
# loop through choice scenarios to create only 12 variables across all blocks
for(i in 1:nrow(d)) {
  if (d$choiceblock[i]==1) {
    d$choice1[i] <- d$block_1_scenario_qn_1[i]
    d$choice2[i] <- d$block_1_scenario_qn_2[i]
    d$choice3[i] <- d$block_1_scenario_qn_3[i]
    d$choice4[i] <- d$block_1_scenario_qn_4[i]
    d$choice5[i] <- d$block_1_scenario_qn_5[i]
    d$choice6[i] <- d$block_1_scenario_qn_6[i]
    d$choice7[i] <- d$block_1_scenario_qn_7[i]
    d$choice8[i] <- d$block_1_scenario_qn_8[i]
    d$choice9[i] <- d$block_1_scenario_qn_9[i]
    d$choice10[i] <- d$block_1_scenario_qn_10[i]
    d$choice11[i] <- d$block_1_scenario_qn_11[i]
    d$choice12[i] <- d$block_1_scenario_qn_12[i]
  } else if (d$choiceblock[i]==2) {
    d$choice1[i] <- d$block_2_scenario_qn_1[i]
    d$choice2[i] <- d$block_2_scenario_qn_2[i]
    d$choice3[i] <- d$block_2_scenario_qn_3[i]
    d$choice4[i] <- d$block_2_scenario_qn_4[i]
    d$choice5[i] <- d$block_2_scenario_qn_5[i]
    d$choice6[i] <- d$block_2_scenario_qn_6[i]
    d$choice7[i] <- d$block_2_scenario_qn_7[i]
    d$choice8[i] <- d$block_2_scenario_qn_8[i]
    d$choice9[i] <- d$block_2_scenario_qn_9[i]
    d$choice10[i] <- d$block_2_scenario_qn_10[i]
    d$choice11[i] <- d$block_2_scenario_qn_11[i]
    d$choice12[i] <- d$block_2_scenario_qn_12[i]
  } else if (d$choiceblock[i]==3) {
    d$choice1[i] <- d$block_3_scenario_qn_1[i]
    d$choice2[i] <- d$block_3_scenario_qn_2[i]
    d$choice3[i] <- d$block_3_scenario_qn_3[i]
    d$choice4[i] <- d$block_3_scenario_qn_4[i]
    d$choice5[i] <- d$block_3_scenario_qn_5[i]
    d$choice6[i] <- d$block_3_scenario_qn_6[i]
    d$choice7[i] <- d$block_3_scenario_qn_7[i]
    d$choice8[i] <- d$block_3_scenario_qn_8[i]
    d$choice9[i] <- d$block_3_scenario_qn_9[i]
    d$choice10[i] <- d$block_3_scenario_qn_10[i]
    d$choice11[i] <- d$block_3_scenario_qn_11[i]
    d$choice12[i] <- d$block_3_scenario_qn_12[i]
  }
}
...
d <- rename(d, c("current_health_status"="health_status",
...
d <-  subset(d, select=c(ID,last_age,
                         ...
                         block,choice1,choice2,choice3,choice4,choice5,choice6,choice7,choice8,choice9,choice10,choice11,choice12,
                         circum_intent,
...
subcols<- c("prob_self_HIV","prob_STDs","prob_partner_HIV","prob_partner_CC","prob_ImproveHygiene","prob_ImproveSexLife",
            "prob_LongerSex","prob_PleasePartner","prob_SexPartner","prob_ReduceSexPleasure","prob_ReduceLibido","prob_Infertile")
...
d$pHIV_C=d$prev_att*d$trans_no_cd*d$prob_self_HIV #/* perceived probability of HIV if circumcised (lower) */
...
sumtab <- rbind(matrix(sapply(d[contvars],subdescfx),nrow=6,byrow=TRUE),matrix(sapply(d[catvars],subdescfx2),nrow=53,byrow=TRUE))
...
names <- c("Age, mean (sd)","Monthly Income in USD, mean (sd)", "Monthly Expenditure in USD, mean (sd)","Absolute Income in USD, mean (sd)","Years of schooling, mean (sd)","No of Children, mean (sd)",
           ...
           "Fisherman","Trading","Farming","Other occupation",
           "Lives in urban area",
...
riskvars <- c("trans_no_cd","trans_wi_cd","prev_all","prev_att",
              ...
              "mort_HIV_pos_tx_10","prob_self_HIV","descriptive3","benefitHIV")
...
desmat <- dummy.data.frame(desmat, names=c("distance1","privacy1","incentive1","distance2","privacy2","incentive2"), sep = "")
desmat <- rename(desmat, c("privacy10"="veryprivate1", "privacy11"="someprivate1", "privacy12"="notprivate1",
                           "privacy20"="veryprivate2", "privacy21"="someprivate2", "privacy22"="notprivate2",
                           "incentive10"="none1", "incentive11"="voucher1", "incentive12"="cash1",
                           "incentive20"="none2", "incentive21"="voucher2", "incentive22"="cash2",
                           "method1"="device1","method2"="device2","method3"="device3",
                           "facility1"="permhc1","facility2"="permhc2","facility3"="permhc3",
                           "distance15"="distance51","distance115"="distance151",
                           "distance21"="distance12","distance25"="distance52","distance215"="distance152"))
